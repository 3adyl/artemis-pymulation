...
- **Dla statku kosmicznego – Rungego-Kutty IV rzędu (RK4):**
  Zaimplementowana w funkcji [rk4](src/physics.py#L24). Metoda RK4 cechuje się błędem lokalnym rzędu $\mathcal{O}(h^5)$ i globalnym $\mathcal{O}(h^4)$. Jest to kluczowe podczas bliskich przelotów obok Księżyca (asysta grawitacyjna), gdzie pole grawitacyjne zmienia się bardzo dynamicznie, a zwykła metoda Eulera prowadziłaby do ogromnych błędów skumulowanych (np. niefizycznego wystrzelenia statku z układu lub zderzenia).
...
- **Dla Księżyca – Euler-Cromer:**
  Zaimplementowana w funkcji [euler_moon](src/physics.py#L46). Ponieważ orbita Księżyca jest idealnie kołowa i nie wpływa na nią lekki statek, do jej opisu wystarczy metoda Eulera-Cromera (pół-niejawna metoda Eulera), która zachowuje energię mechaniczną układu (jest integratorem symplektycznym):
...
- **[src/config.py](src/config.py):** Definiuje stałe fizyczne (masy ciał, promienie orbitalne), parametry początkowe (wysokość orbity parkingowej $185\text{ km}$, domyślne kąty), a także stałe wizualne (skalowanie ciał do wyświetlania w 3D, prędkości początkowe, krok czasowy integracji `DT = 30` sekund).
- **[src/physics.py](src/physics.py):** Zawiera funkcje obliczające przyspieszenia grawitacyjne ([accel_ship](src/physics.py#L6) oraz [accel_moon](src/physics.py#L16)) oraz solvery różniczkowe ([rk4](src/physics.py#L24) i [euler_moon](src/physics.py#L46)).
- **[src/simulation.py](src/simulation.py):** Klasa [Simulation](src/simulation.py#L11) zarządza stanem symulacji (położenie i prędkość statku i Księżyca), wykonuje kroki fizyczne ([step](src/simulation.py#L46)), sprawdza warunki zakończenia misji ([check_termination_conditions](src/simulation.py#L59) – np. ponowne wejście w atmosferę Ziemi lub przekroczenie limitu 13 dni) oraz agreguje dane do telemetrii ([get_telemetry](src/simulation.py#L67)).
- **[src/scene.py](src/scene.py):** Odpowiada za konfigurację sceny 3D biblioteki VPython ([setup_scene](src/scene.py#L11)) – w tym ustawienia kamery – oraz generowanie obiektów 3D reprezentujących Ziemię ([setup_earth](src/scene.py#L30)), Księżyc ([setup_moon](src/scene.py#L53)) i statek kosmiczny wraz z parametrami śladu orbitalnego ([setup_ship](src/scene.py#L80)).
- **[src/controls.py](src/controls.py):** Klasa [Controls](src/controls.py#L6) obsługuje callbacki GUI (obsługa ruchów suwaków parametrów początkowych, zmiana statusów przycisków, przyspieszanie i spowalnianie symulacji).
- **[src/styles.py](src/styles.py):** Definiuje arkusze CSS i kod JavaScript wstrzykiwane bezpośrednio do okna VPython, co umożliwia spersonalizowanie układu graficznego oraz stworzenie nowoczesnego HUD-u w stylu "dark mode".
- **[src/telemetry.py](src/telemetry.py):** Funkcja [format_telemetry](src/telemetry.py#L1) generuje strukturę HTML dla panelu telemetrycznego na podstawie aktualnych parametrów lotu zebranych przez symulator.
- **[src/main.py](src/main.py):** Główna funkcja [main](src/main.py#L17), która inicjalizuje obiekty, buduje interfejs użytkownika z suwakami i przyciskami, a następnie wykonuje pętlę renderowania i aktualizacji fizyki ze stałą częstotliwością odświeżania (60 klatek na sekundę).
...
Zainstaluj wymagane pakiety (`vpython` i `setuptools` do poprawnego działania biblioteki) zdefiniowane w pliku [requirements.txt](requirements.txt):
```bash
...
Projekt jest rozpowszechniany na warunkach licencji MIT. Pełna treść licencji znajduje się w pliku [LICENSE](LICENSE).
